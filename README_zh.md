...
大禹（SylixOS 适配版）是在原大禹系统基础上针对国产 SylixOS 实时操作系统进行深度适配优化的流数据处理自适应调度系统。大禹系统支持多数据流的流水线服务处理，并专注于边缘计算中的调度策略。它基于 KubeEdge 和 ECSM 开发，可在包括 Linux、SylixOS 在内的多种平台之间轻松迁移。
...
大禹（SylixOS 适配版）由五个层次组成，与原版架构基本一致，但增加了对 SylixOS 操作系统的支持：

- **基础系统层**：该层采用 `KubeEdge` 和 `ECSM` 的混合架构，经过适配后可同时部署在 Linux 边缘节点和基于 SylixOS 国产操作系统的边缘节点上。`KubeEdge` 是华为为边缘场景提出的 `Kubernetes` 扩展，而 `ECSM` 用于适配 SylixOS 环境下的容器化应用编排和设备管理。
...
- **基础系统层**
  - `KubeEdge` 使用 CloudCore 和 EdgeCore 实现 Linux 云边环境中的容器化应用编排和设备管理，而 `ECSM` 用于进一步支持基于 SylixOS 的边缘节点。
- **中间接口层**
...
- **兼容异构节点**：平台兼容不同硬件架构（如 x86/arm64）和不同操作系统（包括 Linux 和国产 SylixOS 实时操作系统）、不同性能配置和资源配置的分布式节点，适应节点间不同的物理距离和通信质量。
...
- **国产化适配**：特别针对国产 SylixOS 实时操作系统进行了深度适配和优化，满足国产化替代需求。
