...
# Dayu (SylixOS Adaptation Version)
...
Dayu (SylixOS Adaptation Version) is a stream data processing and adaptive scheduling system that has been deeply adapted and optimized for the domestic SylixOS real-time operating system based on the original Dayu system. Dayu system supports pipeline service processing of multi data stream and focuses on scheduling policies in edge computing. It's developed based on KubeEdge and ECSM and can be easily migrated between multiple platforms including Linux and SylixOS.
...
- [SylixOS](https://www.acoinfo.com/)
- [ECSM](https://www.acoinfo.com/product/cloud-native/ecsm)

## Architecture

Dayu (SylixOS Adaptation Version) consists of five layers, basically consistent with the original architecture but adds support for the SylixOS operating system:

- **Basic System Layer**: This layer adopts the hybrid architecture of `KubeEdge` and `ECSM`, which can be deployed on Linux edge nodes and SylixOS-based domestic OS edge nodes after adaptation. `KubeEdge` is the `Kubernetes` extension proposed by Huawei for edge scenarios, while `ECSM` is used to adapt containerized application orchestration and device management under SylixOS environment.
...
- Basic System Layer
  - `KubeEdge` uses CloudCore and EdgeCore to implement containerized application orchestration and device management in Linux cloud-edge environments, while `ECSM` is used to further support SylixOS-based edge nodes.
...
- **Compatible across heterogeneous nodes**: The platform is compatible with different hardware architectures (such as x86/arm64) and different operating systems (including Linux and the domestic SylixOS real-time operating system), different performance configurations and resource configurations of distributed nodes, adapting to different physical distances and communication quality among nodes.
...
- **Support parallel processing of multiple data streams**: The platform supports parallel processing of multiple data streams (for example, cameras at different intersections simultaneously process traffic flow tasks), these tasks do not distinguish between data streams during the processing stage and are processed equivalently.
- **Domestic Adaptation**: Specifically adapted and optimized for the domestic SylixOS real-time operating system, meeting the requirements of domestic substitution.
