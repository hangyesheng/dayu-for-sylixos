...
class Task:
    ...
    # TODO
    @staticmethod
    def extract_dag_from_dict(dag: dict):
        dag_flow = []
        for in_service in dag:
            ...
            dag_flow.append(service)

        if dag_flow and dag_flow[-1].get_service_name() != 'end':
            dag_flow.append(Service('end'))

        return dag_flow

    # TODO
    @staticmethod
    def extract_dicts_from_dag(dag_flow: dict):
        pipeline_list = []
        for service in dag_flow:
    ...
    def get_pipeline_dicts(self):
        ...
        for service in self.__dag_flow:
            dag = {'service_name': service.get_service_name(),
                   'execute_device': service.get_execute_device()}
            dag_list.append(dag)
    ...
    def set_initial_pipeline(self, dag):
        self.__dag_flow = Task.extract_dag_from_dict(dag)
    ...
    def get_current_service(self):
        service = self.__dag_flow[self.__cur_flow_index]
        return service.get_service_name(), service.get_execute_device()

    def save_transmit_time(self, transmit_time):
        self.__dag_flow[self.__cur_flow_index].set_transmit_time(transmit_time=transmit_time)

    def save_execute_time(self, execute_time):
        self.__dag_flow[self.__cur_flow_index].set_execute_time(execute_time=execute_time)

    def save_real_execute_time(self, real_execute_time):
        self.__dag_flow[self.__cur_flow_index].set_real_execute_time(real_execute_time=real_execute_time)

    def calculate_total_time(self):
        assert self.__dag_flow, 'pipeline of task is empty!'
        assert self.__cur_flow_index < len(self.__dag_flow), 'pipeline is not completed!'
        total_time = 0
        for service in self.__dag_flow:
    ...
    def calculate_cloud_edge_transmit_time(self):
        assert self.__dag_flow, 'dag of task is empty!'
        assert self.__cur_flow_index < len(self.__dag_flow), 'dag is not completed!'
        transmit_time = 0
        for service in self.__dag_flow:
    ...
    def step_to_next_stage(self):
        ...
        assert 0 <= self.__cur_flow_index < len(self.__dag_flow), \
    ...
    def get_current_stage_device(self):
        ...
        return self.__dag_flow[self.__cur_flow_index].get_execute_device()

    def set_initial_execute_device(self, hostname):
        assert self.__dag_flow, 'pipeline is empty!'
        for service in self.__dag_flow:
            service.set_execute_device(hostname)

    @staticmethod
    def set_execute_device(dag, hostname):
    ...
    def serialize(task: 'Task'):
        return json.dumps({
            ...
            'dag': DAG.serialize(task.get_pipeline()),
