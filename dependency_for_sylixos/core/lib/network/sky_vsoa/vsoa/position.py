	...
	def run(self, host: str, port: int) -> None:
		...
		while True:
			...
			if self.__psock in elist:
				buf, remote = self.__psock.recvfrom(MAX_QUERY_BUFFER, socket.MSG_DONTWAIT)
				if buf:
...
def query(name: str, pserver: tuple[str, int] | str, domain: int) -> tuple[str, int]:
	...
	if psock in elist:
		buf, remote = psock.recvfrom(MAX_QUERY_BUFFER, socket.MSG_DONTWAIT)
		psock.close()
...
if platform.system() == 'Windows':
	POS_SERVER_CONF = 'C:\\Windows\\System32\\drivers\\etc\\sky_vsoa.pos'
else:
	POS_SERVER_CONF = '/etc/sky_vsoa.pos'
